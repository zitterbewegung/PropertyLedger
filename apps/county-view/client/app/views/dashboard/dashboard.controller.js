...
.controller('DashboardCtrl', ['$scope', '$http', function ($scope, $http) {
    ...
    if (err) {
      ...
      $scope.chain = response.data.map(function(transaction) {
        ...
        $scope.transactions.push({
          ...
          transaction_submitter: type === 'SetupDemo' ? 'Liam Grace' : 'Broker Order'
        });
  ...
  function openPlaceOrderWebSocket() {
    ...
    placeOrder.onmessage = function(event) {
      ...
      $scope.addBlock(order.transactionId, 'PlaceOrder', 'Broker Order');
      $scope.$apply();
    }
  }
  ...
  function openUpdateOrderWebSocket() {
    ...
    updateOrder.onmessage = function(event) {
      ...
      $scope.addBlock(status.transactionId, 'UpdateOrderStatus', 'Broker Order', status.orderStatus);
      $scope.$apply();
    }
  }
